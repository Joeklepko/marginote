...
- CLI 增加 `--dry-run` 与 `--request-id`，成功写入按请求指纹保留 24 小时幂等结果，JSON 信封返回 `requestId`。
- CLI 笔记/待办列表增加 cursor 分页；笔记支持完整 Markdown 输出或安全导出到文件；受控 `call` 开放批量移动、更新、完成和删除工具。
- CLI 增加离线 `instructions` 命令，可直接输出供 Claude Code/Codex 使用的安全操作规则。
- AI Prompt、Skill、Provider 请求/SSE 解析与助手运行时纯逻辑统一为共享核心；两端显示检索、模型调用、耗时和 token 用量。
- 内置 AI 增加声明式 Skill/Profile 注册表：笔记检索、笔记整理、待办查询、待办管理、持久记忆和通用助手分别声明允许工具、读写策略、最大步骤及专属 Prompt 规则；所有助手工具均有明确归属。
- 写入 Skill 进一步按创建、编辑、整理、完成、删除和批量动作动态收窄权限；新建笔记不再同时开放编辑、批量与删除工具，批量移动/完成也不会获得批量删除能力。
- 桌面端与扩展端共用模型响应解析、工具结果摘要/压缩、上下文压缩和错误降级逻辑；非标准工具 JSON 可安全抢救，推理标签不会进入回复或干扰解析。
- 新增 AI / CLI 共用的工具策略注册表，统一声明 54 个业务工具及 CLI 虚拟命令的读写级别、删除确认、Repository 事务、工作目录投影和暴露范围；桌面、扩展和 Rust CLI schema 均有完整性/一致性回归测试。
- Windows 待办提醒改为常驻系统通知，默认保持到用户处理；通知内可直接选择“1 小时后提醒”“1 天后提醒”或“关闭”，稍后提醒状态会同步回待办并在重启后恢复。
- 桌面版增加 CLI 状态栏，实时显示本地 Agent 当前执行的查询 / 写入任务、运行时间、排队数量以及成功或失败状态。
- AI 与 CLI 共用的非破坏性写工具接入 Repository / UnitOfWork：一次操作只提交一次主数据，失败自动恢复笔记、待办、笔记本和文件夹快照；CLI 复合更新不再出现前半段成功、后半段失败。
- 每次事务写入生成 `cs-...` 变更编号；AI 收据和“数据 · 备份”可查看摘要，IndexedDB 保留最近 30 条完整前后值，ZIP 导出包含 `_change_sets.json`。
- 笔记与待办 AI 快捷动作支持只处理编辑器选中文本，并在原位置预览、提交或失败恢复。
- AI 助手输入区可一键附加当前笔记、待办或选区并显示可移除上下文 chip；对话中的选区按只读上下文处理，防止误覆盖整篇笔记。
- AI 增加通用 `query_notes` 结构化查询，可按全文主题、标题、正文、笔记本、标签、类型、收藏、长度和时间组合条件并分页；条件批量删除基于同一查询冻结确认集合，不再为具体清理语句写专用逻辑。
- 记录型对话会先检索相似主题，可靠命中时优先追加已有笔记，没有匹配才新建；连续对话保留上一轮写入目标 ID，重复段落不会再次追加。

### 修复

- 扩展端补齐双击选词保护、自适应工具栏、宽屏阅读和真实编辑/阅读缩放，与桌面端保持一致。
- 移除 Google Fonts 启动联网，新增根级 `npm run verify`、扩展核心生成校验和 Rust fmt/clippy CI 门禁。
- AI 工具选择从 Prompt 提示升级为运行时强制白名单；移除没有独立权限边界的子代理执行器，桌面版与扩展版的删除笔记、待办、笔记本、批量数据和持久记忆均必须经用户确认。
- 修复 `quick_todo` 未进入 AI 写入事务、番茄钟被当成只读状态，以及扩展端纯文本优化误触发工作目录全量写回等工具分类漂移。
- 修复待办查询意图错误开放创建工具，以及模板创建、批量标题、合并笔记、笔记计数、收藏列表和文本翻译等已实现工具无法从正常 AI 对话到达的问题。
- 修复“明天九点提醒我”等自然表达被误判为待办查询、“查看已完成待办”被误判为写入，以及结束记录后仍用“结束记录”原文规划工具权限的问题。
- 桌面端与扩展端统一通过本轮规划器处理附件纠偏、动作权限和步骤预算；扩展端历史上下文改用统一窗口，避免长会话一次发送最多 200 条消息。
- 工具步骤上限现在同时限制模型轮数和实际 actions 数量，单次响应无法再通过返回大量 actions 绕过预算；扩展端增加重复调用防打转和“声称写入但未实际调用工具”的纠错。
- 取消、接口限流、上下文过长和网络失败使用两端一致的可操作提示，并尽量保留失败前已经生成的部分回复与工具记录。
- 笔记正文、检索结果、待办和持久记忆不再拼入 system 消息，改为独立的不可信本地数据上下文；普通对话不再自动开放 `save_memory`，只有用户明确要求记住时才允许写入。
- 修复 AI 对话附加含图片笔记时没有等待异步图片读取、导致图片未实际发送给多模态模型的问题。
- 桌面 KV 与工作目录文件改为同目录临时文件、刷盘后原子替换；KV 的锁覆盖完整 read-modify-write，并发更新不再互相覆盖，异常退出也不会留下半截 JSON/Markdown。
- 主数据增加 `schemaVersion: 1` 契约；旧数据自动兼容迁移，检测到由未来版本创建的数据时先保护原文并拒绝用旧结构覆盖。
- 工作目录启动扫描增加缺失项对账：用户在磁盘删除的已管理笔记不会再被旧缓存自动写回；笔记进入可恢复软删除、待办移除前生成完整恢复副本，重新放回带相同 ID 的文件可以恢复。
- 工作目录同步改为串行执行并传播读写错误；最近一次异常会显示在“数据 · 备份”，损坏的 `_marginote/meta.json` 不再被静默覆盖。
- 工作目录路径增加符号链接逃逸防护，递归扫描不再跟随可能指向目录外部的链接。
- 修复“准时提醒”因 0 分钟提前量被误判为关闭提醒而无法触发的问题。
- 主数据损坏或结构异常时先保留原始恢复副本，备份未完成前阻止覆盖，避免启动时静默清空数据；恢复副本会随 ZIP 导出到 `_recovery/`。
- 修复 ZIP 导入图片只保存在内存、重启后丢失的问题；导入完成前会等待图片写入 IndexedDB。
- AI 流式改写改为“草稿 → 成功后一次提交”，请求失败不再留下半截内容，生成期间切换笔记也不会误写新目标。
- CLI 高级 `call` 删除工具同样强制要求 `--yes`，并在前端业务桥再次校验确认标记。
- 修复 Chrome 扩展入口引用错误的平台脚本路径，并增加静态资源完整性测试。
- 多文件导入改为整批失败回滚，处理底层读取错误，清理失败批次新增图片，并限制 ZIP 输入、文件数与解压体积，避免半份恢复和压缩炸弹造成内存失控。
- 笔记/待办/主页面空状态增加直接新建或清除筛选操作；搜索输入增加防抖。
- 修复扩展端遗漏事务式 AI 编辑核心、流式过程中直接改真实数据、生成期间切换目标误写，以及保存包装器丢失失败返回值的问题。
- 用户直接询问事实时会主动检索本地笔记，无需显式说“查找”；修复“提醒功能/任务调度”等笔记主题被宽泛关键词误判为个人待办，以及“记下来/写个笔记/保存到记忆”等自然表达路由不稳定的问题。
...
- Skill 文档 `docs/ai-skills.md` 描述 AI 助手能力规格、工具协议和安全边界
