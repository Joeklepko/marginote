...
- **画板恢复可用**：找回被误丢弃的「Excalidraw 资源内置入库」——之前 `vendor/` 为空导致画板加载不到运行时、显示"资源没装"、无工具栏。现把运行时+字体+语言包（约 4.4MB）随包分发，克隆/安装即用、离线可用，无需再跑 `fetch.sh`。
- **画板可删除**：画板视图右上角新增「🗑 删除」按钮（此前打开画板后无处删除该文件）；绑定工作目录时删除进回收站可恢复。
