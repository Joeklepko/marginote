...
- **删除/移动笔记后工作目录旧文件残留导致"复活"**：删除笔记本、删除/移动笔记时，写回工作目录只写当前文件、不清理旧路径，残留的 `.md` 会在下次导入时被读回、重建已删的笔记本/笔记。改为写回时按 meta 记录的 `noteFiles`/`todoFiles` 对账清理「上次写过、这次不再占用」的旧路径（移动/重命名/删除/删笔记本均覆盖）；只删 Marginote 自己写过的路径，不碰用户外部新增文件，路径互换也不误删
